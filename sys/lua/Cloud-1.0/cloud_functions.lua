...
function chat(id, text) -- @TODO: Recreate a dynamic (yet not buggy) chat function
	...
	if _PLAYER[usgn] and cloud.player[id].say ~= 0 then
		c = (_PLAYER[usgn].colour or _GROUP[(_PLAYER[usgn].group or cloud.setting.group_default)].colour)
		p = ((_PLAYER[usgn].prefix or _GROUP[(_PLAYER[usgn].group or cloud.setting.group_default)].prefix) or "")
	else
		c = (_GROUP[cloud.setting.group_default].colour or "")
		p = (_GROUP[cloud.setting.group_default].prefix or "")
	end

	if cloud.player[id].pre == 0 then
		p = ""
	end
	...
	if cloud.player[id].prefix then
		msg(c..p..color..name..": "..clr["cloud"]["chat"]..text)
	else
		msg("\169".._GROUP.default.colour.._GROUP.default.prefix.." "..color..name..": "..clr["cloud"]["chat"]..text)
	end
end
...
function setConsoleHelp(func, info)
	if not info then info = "" end
	cloud.console.help[func] = func.." "..info
end
...
function cloudMessage(id, text, type, prefix)
	...
	if clr["cloud"][type] then
		colour = clr["cloud"][type]
	end
	...
	if prefix == nil then
		pre = (cloud.setting.message_prefix or prefix)
	else
	...
	local message = colour..pre..clr["cloud"]["default"]..text
...
function cloudPrint(text, type, prefix)
	if not type then type = "default" end
	if not text then return 1 end -- This is so things don't go batshitmad
...
function cloudLog(log, file, extension, type, date)
	if not file or file == "" then
		file = "log"
		if date == true then
			file = cloud.setting.date
		end
	else
		if date == true then
			file = file.."-"..cloud.setting.date
		end
	end
	...
	file:write(cloud.setting.date.." - "..cloud.setting.time.."\n")
	file:write(log.."\n")
	file:close()
end
...
function executeCommand(id, command, text, mode)
	...
	func = loadstring("cloud.func."..mode.."."..command.."()")
	func()

	if id then
		cloudLog("[ID: "..id.."] [USGN: "..player(id, "usgn").."] [IP: "..player(id, "ip").."] [Team: "..player(id, "team").."] [Name: "..player(id, "name").."]: "..text, cloud.setting.date, ".txt", "a")
	end
...
function setUndo(id, command)
	if not checkUsgn(id, false) then
		cloudMessage(id, "An undo command cannot be set as you are not logged in to a U.S.G.N account.", "alert")
		return false
	end
...
function checkPlayer(id, message)
	...
	if not id or tonumber(id) == nil then
		if message then
			cloudMessage(_id, "You have not supplied a player id!", "warning")
		end
		return false
	end
...
function checkUsgn(id, message)
	...
	if player(id, "usgn") == 0 then
		if message then
			cloudMessage(_id, "This player does not have a U.S.G.N. ID!", "warning")
		end
		return false
	end
...
function compareLevel(id, id2)
	...
	local lvl = _GROUP[cloud.setting.group_default].level or 0
	local lvl2 = _GROUP[cloud.setting.group_default].level or 0

	if _PLAYER[usgn] and _PLAYER[usgn].group then
		lvl = _GROUP[(_PLAYER[usgn].group or cloud.setting.group_default)].level or 0
	...
	if _PLAYER[usgn2] and _PLAYER[usgn2].group then
		lvl2 = _GROUP[(_PLAYER[usgn2].group or cloud.setting.group_default)].level or 0
...
function softReload()
	skipSoftReload = true
	dofile(_DIR.."cloud_startup.lua")
	-- dirty fix (log, second, ms100) crash the server
	timer(3000,"sreload")
	function sreload()
		skipSoftReload = nil
	end
...
function checkInitialAuth()
	if (_CLOUD.auth_token ~= false and _CLOUD.auth_token == true) then
		print(clr["cloud"]["default"].."[CLOUD]: Initial authentication U.S.G.N: ".._CLOUD.auth_usgn)
		return true
	end
	_CLOUD.auth_token = createToken(5)
	print(clr["cloud"]["warning"].."[CLOUD]: Initial authentication has not been complete.")
	print(clr["cloud"]["warning"].."[CLOUD]: Please say !auth ".._CLOUD.auth_token)
	return false
end
...
function editGroup(group, field)
	...
	if _tbl[5]:sub(1, 1) == "/" then
		_GROUP[group][field] = cloud.constant[_tbl[5]:sub(2)]
	else
	    if t == "table" then
...
function editPlayer(player, field)
	...
	if _tbl[5]:sub(1, 1) == "/" then
		_PLAYER[player][field] = cloud.constant[_tbl[5]:sub(2)]
	else
	    if t == "table" then
...
function checkCommand(command, mode)
	local tmp = {}
	for k, v in pairs(cloud.func[mode]) do
		tmp[k] = k
	end

	for cmds = 1, countIterate(cloud.func[mode]) do
		if tmp[command] then
...
function checkSayCommandUse(command)
	if not _CLOUD.disabled_commands then _CLOUD.disabled_commands = {} end
	if not command then
		cloudPrint("No command was provided to check if the use of it is allowed!", "warning")
		return true
	end
...
function addFilter(name, func, priority)
    if not cloud.filter[name] then cloud.filter[name] = {} end
    if priority then table.insert(cloud.filter[name], priority, func)
    else table.insert(cloud.filter[name], func) end
end
...
function filter(name, ...)
	if cloud.filter[name] then
	    local f, l = table.bounds(cloud.filter[name])
	    for i = f, l do
	        local func = cloud.filter[name][i]
...
function addAction(name, func, priority)
    if not cloud.action[name] then cloud.action[name] = {} end
    if priority then table.insert(cloud.action[name], priority, func)
    else table.insert(cloud.action[name], func) end
end
...
function action(name, ...)
	if cloud.action[name] then
	    local f, l = table.bounds(cloud.action[name])
	    for i = f, l do
	        local func = cloud.action[name][i]
...
function checkStatus()
	local checkstatus = io.popen("curl -Is https://github.com/AlexSomba/Cloud-v1.0.git | head -1")
...
function checkVersion()
	if not cloud.setting.check_version then
		cloudPrint("Version check is disabled. Please enable this to stay up-to-date in cloud_config.lua", "warning")
		return
	end
	if not checkStatus() then
		cloudPrint("No connection status could be made with https://github.com/AlexSomba/Cloud-v1.0.git", "warning")
		return
	end

	handle = io.popen("curl https://github.com/AlexSomba/Cloud-v1.0.git")
	local git_version = tostring(handle:read("*a"))
	local local_version = tostring(cloud.version)
	handle:close()
	...
	if git_version > local_version then
		cloudPrint("You are not up-to-date with the current version!", "warning")
		cloudPrint("Please download the current version at http://github.com/AlexSomba//docs", "warning")
	elseif git_version < local_version then
		cloudPrint("You are running on a higher version of the current release. Huh? I don't even..", "alert")
	else
		cloudPrint("You are up-to-date with the current version!", "success")
	end
end
	...
	for _, all in pairs(directories) do
		if all:sub(1, 1) ~= "." then
	  		if all:sub(1, 1) ~= "_" then
	  			...
	  			cloudPrint("Loading plugin "..all.."..", "success", "[PLUGIN]: ")
	  			cloud.plugin[all] = {}
	  			cloud.plugin[all]["dir"] = _DIR.."plugins/"..all.."/"
	  			dofileLua(cloud.plugin[all]["dir"].."/startup.lua")
	  			cachePluginData()
...
function cachePluginData()
	for k, v in pairs(cloud.plugin) do
		_PLUGIN["info"][k] = {}
...
function checkPluginData(name, data, varType)
	if cloud.plugin[name] then
		if cloud.plugin[name][data] and type(cloud.plugin[name][data]) == varType then
			_PLUGIN["info"][name][data] = cloud.plugin[name][data]
		else
			cloudPrint("Plugin information for "..data.." not set or is not a "..varType.."!", "alert", "[PLUGIN]: ")
		end
	end
end
...
function dofileLua(path, create)
	if not fileExists(path) then
		if create == true then
			cloudPrint("Uh-oh! The file '"..path.."' could not be found or opened, creating one for you instead!", "alert")
			file = io.open(path, "w")
			io.close(file)
		else
			cloudPrint("Uh-oh! The file '"..path.."' could not be found or opened!", "warning")
			return false
		end
	end
	dofile(path)
...
function checkForceReload()
	if cloud.force_reload == true then
		cloudMessage(false, "A plugin has been enabled which requires a server restart, please stay!", "success")
...
function addTransferFile(file, path)
	if not file then
		cloudPrint("No file name was defined to add to the server transfer list.", "warning")
		return false
	end

	if not path then
		cloudPrint("No file path was defined to use to add a file to the server transfer list.", "warning")
		return false
	end

	local _, count = string.gsub(file, "%.", "")
	if count < 1 then
		cloudPrint("The file '"..path..file.."' cannot be added to the server transfer list as it does not have an extension!", "warning")
		return false
	end
	if count > 1 then
		cloudPrint("The file '"..path..file.."' cannot be added to the server transfer list as it contains more than one dot!", "warning")
		return false
	end

	if not fileExists(path..file) then
		cloudPrint("The file '"..path..file.."' cannot be added to the server transfer list as it does not exist!", "warning")
		return false
	end

	table.insert(cloud.transferlist, path..file)
	return true
end

--[[
	Adds all the files in cloud.transferlist to the server transfer list
	@return boolean
]]
function setTransferList(response)
	table.fileToTable("sys/servertransfer.lst", cloud.transferlist)

	local file = io.open("sys/servertransfer.lst", "w+") or io.tmpfile()
	local count = 0

	cloud.transferlist = table.removeDuplicate(cloud.transferlist)

	for k, v in pairs(cloud.transferlist) do
		local text = v.."\n"
		file:write(text)
		count = count + 1
		if response then
			cloudPrint("The file '"..v.."' has been added to the server transfer list.", "success")
		end
	end
	file:close()

	if count > 0 then
		cloudPrint("The server transfer list has been updated. Please restart your server if necessary.", "info")
	end
end
